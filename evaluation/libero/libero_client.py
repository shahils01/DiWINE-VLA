...
class ClientModel:
    ...
    def _format_query(self, obs: Dict, goal: str) -> Dict:
        ...
        robo_ori = obs["robo_ori"]
        if isinstance(robo_ori, np.ndarray) and robo_ori.shape[-1] == 6:
            rot6d = robo_ori
        else:
            # Convert axis-angle -> rot6d for proprio to match training
            rot6d = self.processor.AxisAngle_to_Rotate6D(robo_ori)
        closed_loop_proprio = np.concatenate([obs["robo_pos"], rot6d, np.array([0.0])], axis=-1)
