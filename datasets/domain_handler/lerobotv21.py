    ...
    def iter_episode(self, traj_idx: int, *, num_actions: int, training: bool,
        ...
        for idx in idxs:
            ...
            if future_offset_steps > 0:
                future_idx = min(idx + future_offset_steps, images[0].shape[0] - 1)
                future_imgs = []
                for v in range(min(self.num_views, len(images))):
                    future_imgs.append(image_aug(Image.fromarray(images[v][future_idx])))
                while len(future_imgs) < self.num_views: future_imgs.append(torch.zeros_like(future_imgs[0]))
                future_image_input = torch.stack(future_imgs, 0)
            cur = t[idx]
            ...
            if future_offset_steps > 0 and future_image_input is not None:
                future_proprio = cur_action[min(future_offset_steps, cur_action.shape[0] - 1)]
