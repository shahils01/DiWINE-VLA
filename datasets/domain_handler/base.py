...
class BaseHDF5Handler(DomainHandler):
    ...
    def _pil_from_arr(arr: Any) -> Image.Image:
        ...
        if isinstance(arr, np.ndarray):
            ...
            if arr.ndim == 1:
                return decode_image_from_bytes(arr)
        return decode_image_from_bytes(arr)
    ...
    def iter_episode(
        ...
        **kwargs
        ...
        for idx in idxs:
            ...
            future_image_input = None
            future_image_mask = None
            future_proprio = None
            if future_offset_steps > 0 and future_horizon_steps > 0:
                future_indices = [
                    min(idx + future_offset_steps + h, images[0].shape[0] - 1)
                    for h in range(future_horizon_steps)
                ]
                future_imgs_seq = []
                for future_idx in future_indices:
                    future_imgs = [image_aug(self._pil_from_arr(images[v][future_idx])) for v in range(V)]
                ...
                future_image_input = torch.stack(future_imgs_seq, dim=0)
                future_image_mask = image_mask.unsqueeze(0).expand(future_horizon_steps, -1)

                future_times = ref[future_indices]
                future_lseq = torch.tensor(L(future_times))
                future_rseq = torch.tensor(R(future_times))
                future_proprio = torch.cat([future_lseq, future_rseq], -1).float()
            ...
            if future_image_input is not None:
                sample["future_image_input"] = future_image_input
